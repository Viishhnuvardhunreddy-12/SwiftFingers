...
import { getDailyActivityFromSupabase, getWeakestKeysFromSupabase } from '../services/analyticsService';
...
const AnalyticsModal: React.FC<AnalyticsModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      if (isOpen) {
        setLoading(true);
        console.log('Loading analytics...', { isSignedIn, userId: user?.id });

        try {
          if (isSignedIn && user?.id) {
            // Fetch from Supabase for signed-in users
            console.log('Fetching from Supabase...');
            const [daily, weak] = await Promise.all([
              getDailyActivityFromSupabase(user.id),
              getWeakestKeysFromSupabase(user.id)
            ]);
            setDailyStats(daily);
            setWeakKeys(weak);
          ...
          setDailyStats(getDailyActivity(user?.id));
          setWeakKeys(getWeakestKeys(user?.id));
        } finally {
          setLoading(false);
        }
      }
