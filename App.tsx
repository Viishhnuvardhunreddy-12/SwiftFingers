...
import { saveSessionToSupabase } from './services/supabaseService';
...
const App: React.FC = () => {
  ...
  const { user, syncStatus } = useSupabaseSync();
  ...
  const handleTypingComplete = async (typedText: string, timeTaken: number) => {
    ...
    try {
      ...
      // SAVE HISTORY for Analytics (localStorage for backward compatibility)
      ...
      if (user) {
        try {
          ...
          console.error('Failed to save to Supabase, but continuing:', supabaseError);
      ...
      <SignedIn>
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
             ...
             {/* User Button - User is already authenticated */}
             <UserButton 
