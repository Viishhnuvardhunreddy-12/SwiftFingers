...
const calculateLocalStats = (original: string, typed: string, timeTaken: number) => {
  ...
  // ── Character-level analysis (for weak-key heatmap) ─────────────────────
  ...
  // Net WPM   = Gross WPM - (errors / minutes)  [errors = wrong words]
  const safeTime = Math.max(timeTaken, 1); // avoid division by zero
  const minutes = safeTime / 60;
  const grossWPM = Math.round((typed.length / 5) / minutes);
  const errorPenalty = Math.round(mistypedWords.length / minutes);
  const netWPM = Math.max(0, grossWPM - errorPenalty);

  // ── Accuracy ─────────────────────────────────────────────────────────────
  // Based on characters: correct chars / total original chars attempted
  let correctChars = 0;
  for (let i = 0; i < minLen; i++) {
    if (typed[i] === original[i]) correctChars++;
  }
  const charsAttempted = Math.max(typed.length, 1);
  const accuracy = Math.max(0, Math.min(100, Math.round((correctChars / charsAttempted) * 100)));

  // ── Score (0–100) ────────────────────────────────────────────────────────
  // Weighted: 60% accuracy + 40% speed (normalised to 120 WPM ceiling)
  const speedNorm = Math.min(netWPM / 120, 1) * 100;
  const score = Math.min(100, Math.round((accuracy * 0.6) + (speedNorm * 0.4)));
